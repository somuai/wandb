...
- LEET links console logs to the charts in the single-run view: with the logs pane focused, press `l` and the charts show a crosshair at the time the selected line was logged (the nearest step on metrics charts, the exact time on system charts) as you move through the log; inspecting a chart with the mouse selects the console line logged nearest to that point (@dmitryduev in https://github.com/wandb/wandb/pull/12737)
